...
import { ensureCodexInstructions, ensureCodexPayloadCompatibility, extractForcedAuthType, extractModelName, normalizeModel, rewriteOpenAIModelsForRouting, selectAuthTypeForRequest, stripForcedAuthType, toCodexBackendUrl } from './routing.js';
...
function hasProviderModelShape(models) {
...
function parseModelNumber(value, fallback = 0) {
...
function toProviderSeedModel(modelID, model) {
...
function buildInjectedLatestModel(providerShape, latestModel) {
    if (providerShape) {
        return {
            id: latestModel,
            providerID: PROVIDER_ID,
...
function replaceModelMapInPlace(target, next) {
            ...
            async loader(getAuth, provider) {
                ...
                if (provider && typeof provider === 'object') {
                    ...
                    if (models && typeof models === 'object') {
                        ...
                        provider.models = replaceModelMapInPlace(models, rewritten);
                    }
                }
                ...
                const customFetch = async (input, init, local429RetryAttempt = 0) => {
                    ...
                    try {
                        ...
                        if (process.env.OPENCODE_MULTI_AUTH_DEBUG === '1') {
                            console.log(`[multi-auth] response auth=${resolvedAuthType} alias=${account.alias} status=${res.status}`);
                        }
