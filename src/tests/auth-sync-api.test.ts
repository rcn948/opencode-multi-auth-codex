...
function setupTempStore(): { dir: string; storeFile: string; cleanup: () => void } {
...
test('syncAuthFromOpenCode imports and deduplicates API keys', async () => {
  ...
  try {
    ...
    await syncAuthFromOpenCode(async () => ({ type: 'api', key: 'sk-one' } as any))
    ...
    __resetAuthSyncStateForTests()
    await syncAuthFromOpenCode(async () => ({ type: 'api', key: 'sk-two' } as any))
    store = loadStore()
    assert.equal(Object.keys(store.accounts).length, 2)

    process.env.OPENCODE_MULTI_AUTH_SYNC_API = '0'
    __resetAuthSyncStateForTests()
    await syncAuthFromOpenCode(async () => ({ type: 'api', key: 'sk-three' } as any))
    store = loadStore()
    assert.equal(Object.keys(store.accounts).length, 2)
  } finally {
    temp.cleanup()
...
test('syncAuthFromOpenCode does not cooldown after empty auth', async () => {
  ...
  try {
    __resetAuthSyncStateForTests()
    await syncAuthFromOpenCode(async () => null as any)
    await syncAuthFromOpenCode(async () => ({ type: 'api', key: 'sk-after-empty' } as any))
